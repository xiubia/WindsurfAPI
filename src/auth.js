...
import { isDatabaseAccountStoreEnabled, loadAccountsFromDatabase, saveAccountsToDatabase } from './account-store.js';
...
function saveAccounts() {
  ...
  const serialized = _serializeAccounts();
  const persist = async () => {
    if (isDatabaseAccountStoreEnabled()) {
      await saveAccountsToDatabase(serialized);
      return;
    }
    ...
    writeFileSync(tempFile, JSON.stringify(serialized, null, 2));
...
export function saveAccountsSync() {
  ...
  if (isDatabaseAccountStoreEnabled()) {
    log.warn('Shutdown: DATABASE_URL enabled, skip sync account flush and rely on prior async saves');
    return;
  }
  try {
...
async function loadAccounts() {
  try {
    let data = [];
    if (isDatabaseAccountStoreEnabled()) {
      data = await loadAccountsFromDatabase();
    } else {
      if (!existsSync(ACCOUNTS_FILE)) return;
      data = JSON.parse(readFileSync(ACCOUNTS_FILE, 'utf-8'));
    }
    for (const a of data) {
    ...
    if (data.length > 0) {
      log.info(`Loaded ${data.length} account(s) from ${isDatabaseAccountStoreEnabled() ? 'database' : 'disk'}`);
    }
  } catch (e) {
